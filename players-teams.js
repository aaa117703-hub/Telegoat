...
function normalizePlayerName(name) {
    ...
    return name
        ...
        .replace(/[\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}\u{1F700}-\u{1F77F}\u{1F780}-\u{1F7FF}\u{1F800}-\u{1F8FF}\u{1F900}-\u{1F9FF}\u{1FA00}-\u{1FA6F}\u{1FA70}-\u{1FAFF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{1F1E6}-\u{1F1FF}]/gu, '')
        .trim()
        .toLowerCase();
}
...
// جلب صورة القميص مع الـ scale المخصص
function getShirtForTeam(teamName) {
    if (!teamName) return { url: './default-shirt.png', scale: 1 };
    
    const cleanTeam = teamName.toLowerCase().trim();
    const shirtFile = TEAMS_SHIRTS[cleanTeam];
    
    if (!shirtFile) return { url: './default-shirt.png', scale: 1 };

    // القمصان التي تحتاج تكبير (Man City & Spurs)
    let scale = 1;
    if (cleanTeam.includes('man city') || cleanTeam.includes('spurs') || cleanTeam.includes('tottenham')) {
        scale = 1.2;
    }
